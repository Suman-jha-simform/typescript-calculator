...
class Calculator {
    ...
    concatnumber(texttoconcat:string) { 
    ...
    operatorconcat(operatortoconcat:string) {
        let operators = /[+\-*/]/;
    ...
    equalsto() {
        ...
        if(this.previousInput.innerText.includes('^') && this.currentInput.innerText != '') {
            ...
            if(this.currentInput.innerText != '' ) {
                ...
                this.currentInput.innerText = eval(this.previousInput.innerText.toString() + this.currentInput.innerText.toString());
                this.previousInput.innerText ='';
            }
        }
    }
    ...
    factorial() {
        if(this.currentInput.innerText != '') {
            if(this.currentInput.innerText.includes('-')) {
                ...
                this.currentInput.innerText = fact ;
            }
       }   
    }

    // function to raise a number by certain power
    powerraise(type: string) {
        if(type == 'normal') {
            if(this.currentInput.innerText != '') {
    ...
    memoryfunctions(type: string) {
        ...
        if(this.currentInput.innerText != '') {
            if(type == 'ms') {
                ...
                let memorystore = Number.parseFloat(this.currentInput.innerText) + Number.parseFloat(localStorage.getItem('memorystore'));
                localStorage.setItem('memorystore', JSON.stringify(memorystore));

            } else if(type == 'sub') {
                let memorystore =  Number.parseFloat(localStorage.getItem('memorystore')) - Number.parseFloat(this.currentInput.innerText);
                localStorage.setItem('memorystore', JSON.stringify(memorystore));
            }
    ...
    remove() {
        if(this.currentInput.innerText != '') {
            let removeable = this.currentInput.innerText.toString().slice(0, -1);
            this.currentInput.innerText = removeable;
        } else if(this.currentInput.innerText == '' && this.previousInput.innerText != '')
        {
            let removeable = this.previousInput.innerText.toString().slice(0, -1);
            this.currentInput.innerText = removeable;
            this.previousInput.innerText ='';
        }
    }
...
memorystore.addEventListener('click', () => {
    ...
    if(flag == 1) {
        (memoryrecall as any).disabled = false;
        (memoryclear as any).disabled = false;
        (datamemory as any).disabled = false;
    }
})

memoryclear.addEventListener('click', () => {
    (memoryrecall as any).disabled = true;
    (memoryclear as any).disabled = true;
    (datamemory as any).disabled = true;
...
number.forEach(button => {
    button.addEventListener('click', () => {
        calculator.concatnumber((button as any).innerText);
    });
});
...
operator.forEach(button => {
    button.addEventListener('click', (e) => {
        calculator.operatorconcat((e.target as any).value);
    });
});
...
roundoff.forEach(button => {
    button.addEventListener('click', () => {
        calculator.roundoff((button as any).value);
    });
});

trifun.forEach(button => {
    button.addEventListener('click', ()=>{
        calculator.trifun((button as any).innerText);
    });
});
...
oparenthesis.addEventListener('click' ,() => {
    calculator.concatnumber((oparenthesis as any).innerText);
});

cparenthesis.addEventListener('click' ,() => {
    calculator.concatnumber((cparenthesis as any).innerText);
});
...
dot.addEventListener('click' ,() => {
    calculator.concatnumber((dot as any).innerText);
});
